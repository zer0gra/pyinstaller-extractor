...
class PyInstArchive:
    ...
    def parseTOC(self):
        ...
        while parsedLen < self.tableOfContentsSize:
            ...
            if len(name) == 0:
                name = str(uniquename())
                print('[!] Warning: Found an unnamed file in CArchive. Using a random name {0}'.format(name))
    ...
    def extractFiles(self):
        ...
        for entry in self.tocList:
            ...
            if entry.typeCmprsData == b's':
                ...
                # Entry points are expected to be python scripts
...
class PyInstallerExtractorGUI:
    def __init__(self, master):
        self.master = master
        master.title("PyInstaller Extractor")

        self.label = tk.Label(master, text="Select a PyInstaller Executable:")
        self.label.pack(pady=10)

        self.browse_button = tk.Button(master, text="Browse", command=self.browse)
        self.browse_button.pack()

        self.extract_button = tk.Button(master, text="Extract", state=tk.DISABLED, command=self.extract)
        self.extract_button.pack(pady=10)

    def browse(self):
        file_path = filedialog.askopenfilename(title="Select a PyInstaller Executable", filetypes=[("Executable files", "*.exe")])
        if file_path:
            self.extract_button["state"] = tk.NORMAL
            self.py_inst_archive = PyInstArchive(file_path)

    def extract(self):
        self.extract_button["state"] = tk.DISABLED
        Thread(target=self.extract_in_thread).start()

    def extract_in_thread(self):
        if self.py_inst_archive.open():
            if self.py_inst_archive.checkFile():
                if self.py_inst_archive.getCArchiveInfo():
                    self.py_inst_archive.parseTOC()
                    self.py_inst_archive.extractFiles()
                    self.py_inst_archive.close()
                    self.show_message("Extraction Complete", "Successfully extracted PyInstaller archive.")
                else:
                    self.show_message("Error", "Failed to get CArchive information.")
            else:
                self.show_message("Error", "Failed to check the PyInstaller executable.")
            self.extract_button["state"] = tk.NORMAL
        else:
            self.show_message("Error", "Failed to open the PyInstaller executable.")
...
if __name__ == '__main__':
    root = tk.Tk()
    app = PyInstallerExtractorGUI(root)
    root.mainloop()
